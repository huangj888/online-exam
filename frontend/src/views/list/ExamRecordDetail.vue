<template>
  <a-layout>
    <a-layout-header class="header" style="color: #fff">
      ...
      <span style="font-size:25px;margin-left: 0px;" v-if="examDetail.exam">
        <a-avatar
          ...
          size="large"
          shape="circle"
          :src="examDetail.exam.examAvatar | imgSrcFilter"
        />
        {{ examDetail.exam.examName }}
        <span
          style="font-size:15px;"
        >{{ examDetail.exam.examDescription }}</span>
      </span>
      <span style="float: right;">
        <span style="margin-right: 40px; font-size: 20px" v-if="recordDetail.examRecord">
          考试得分：
          <span style="color: red">{{ recordDetail.examRecord.examJoinScore }}</span>&nbsp;分&nbsp;
          <span
            style="font-size:15px;"
          >参加考试时间：{{ recordDetail.examRecord.examJoinDate }}</span>
        </span>
    ...
    <a-layout>
      <a-layout-sider
        width="190"
        :style="{background: '#444',overflow: 'auto', height: '100vh', position: 'fixed', left: 0 }"
      >
        <a-menu
          mode="inline"
        ...
        >
          ...
          <a-sub-menu key="question_check">
            <span slot="title" v-if="examDetail.exam">
              ...
              多选题(每题{{ examDetail.exam.examScoreCheck }}分)
            </span>
            <a-menu-item
      ...
      <a-layout :style="{ marginLeft: '200px' }">
        <a-layout-content :style="{ margin: '24px 16px 0',height: '84vh', overflow: 'initial' }">
          <div :style="{ padding: '24px', background: '#fff',height: '84vh'}">
            <span
              ...
              style="font-size: 30px;font-family: Consolas"
            >欢迎查看本次考试情况，点击左侧题目编号可以查看答题详情</span>
            ...
            >
              ...
              >{{ option.questionOptionContent }}</a-radio>
            </a-radio-group>

            <!-- 题目出错的时候才显示这块 -->
            <div
              v-if="!questionRight && currentQuestion!=='' && (currentQuestion.type === '单选题' || currentQuestion.type === '判断题')"
            >
            ...
            <a-checkbox-group v-model="checkValues" v-if="currentQuestion.type === '多选题'">
              <a-checkbox
                ...
                :style="optionStyle"
                :value="option.questionOptionId"
              >{{ option.questionOptionContent }}</a-checkbox>
            </a-checkbox-group>
        ...
        <a-layout-footer :style="{ textAlign: 'center' }">Copyright © 2023 guang dong bai yun</a-layout-footer>
      </a-layout>
    </a-layout>
  </a-layout>
</template>
